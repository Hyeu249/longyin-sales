...
import { usePurchaseReceipt, RFID, Item } from "./PurchaseReceiptContext";
...
export default function RFIDsList() {
  ...
  useEffect(() => {
    const initData = async () => {
      const data = await db.getDocList("RFID Tag", {
        fields: ["name", "item", "gas_serial_no", "warehouse", "customer"],
        limit: 10000,
      });
  ...
  const updateRfids = (rfids: any = []) => {
    ...
    setRecord({
      ...
      supplier: cRecord?.supplier || "",
      currency: cRecord?.currency || "",
      buying_price_list: cRecord?.buying_price_list || "",
      conversion_rate: cRecord?.conversion_rate || 0,
