...
import { useStockEntry, RFID, Item } from "./StockEntryContext";
...
export default function RFIDsList() {
  ...
  const check_logic_transfer = (rfid_tag: any): boolean => {
    ...
    const transfer_type = cRecord?.stock_entry_type === "Material Transfer";
    const in_stock = rfid_tag.warehouse === cRecord?.from_warehouse;
  ...
  const updateRfids = (rfids: any = []) => {
    ...
    setRecord({
      ...
      stock_entry_type: cRecord?.stock_entry_type || "",
      from_warehouse: cRecord?.from_warehouse || "",
      to_warehouse: cRecord?.to_warehouse || "",
      items: new_items,
      rfids: rfids,
    });
  };
